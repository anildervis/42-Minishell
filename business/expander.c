...
int	wild_path(char *wild_one, char *expected_one)
{
	...
	tmp_wild_one += i + 1;
	tmp_expected_one += i;
    if (!wild_path_helper(star_count, &tmp_wild_one, &tmp_expected_one))
        return (0);
	if (ft_strlen(tmp_expected_one) < ft_strlen(tmp_wild_one))
		return (0);
	else
		tmp_expected_one += ft_strlen(tmp_expected_one)
			- ft_strlen(tmp_wild_one);
	return (!ft_strcmp(tmp_wild_one, tmp_expected_one));
}
...
void	expander(t_parsed **command)
{
	...
	while ((*command)->arguments[++i])
	{
		...
		if (ft_strnsearch((*command)->arguments[i], "*",
				ft_strlen((*command)->arguments[i])))
			continue ;
		wildcard(".", ft_split((*command)->arguments[i], '/'), 0,
				&wildcard_list);
		tmp_arguments = ft_calloc(list_len((*command)->arguments)
