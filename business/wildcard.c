...
int	checking_between_wildcards(int wildcard_count, char *wild_str, char *expected_str)
{
    ...
    while (wildcard_count-- - 1 > 0)
    {
        ...
        while (wild_str[i] != *WILD_CARD)
            i++;
		if (!(expected_str = strnstr_wildcard(expected_str, wild_str, i)))
            return 0;
        wild_str += i + 1;
        expected_str += i;
    }
    ...
    return !ft_strcmp(wild_str, expected_str); // comparing the part after the last wildcard
}
...
int	is_wild_eq_string(char *wild_str, char *expected_str)
{
	...
	i = 0;
    if (!(count = wildcard_count(wild_str))) // getting wildcard count
        return !ft_strcmp(wild_str, expected_str);
    i = 0;
    while (wild_str[i] && wild_str[i] != *WILD_CARD) // getting the index of first wildcard
        i++;
    if (i != 0 && ft_strncmp(wild_str, expected_str, i)) // comparing the part before the first wildcard
            return 0;
    wild_str += i + 1;
    expected_str += i;
	if (!checking_between_wildcards(count, wild_str, expected_str)) // checking if between the wildcards are correct
		return 0;
	return 1;
}
